	...
	public partial class CExplorerBrowserHostDialog : Form, IServiceProvider, ICommDlgBrowser3
	{
		...
		private void _OnInitDlg(object sender, EventArgs e)
		{
			...
			try
			{
				...
				{
					var rgkeys = new[] { Ole32.PROPERTYKEY.System.ItemNameDisplay, Ole32.PROPERTYKEY.System.ItemFolderPathDisplay };
		...
		private void _OnSelChange()
		{
			if (_fEnumerated)
			{
				...
				IDC_FOLDERNAME.Text = (string)psi?.GetDisplayName(SIGDN.SIGDN_NORMALDISPLAY) ?? "";
				IDC_FOLDERPATH.Text = (string)psi?.GetDisplayName(SIGDN.SIGDN_DESKTOPABSOLUTEPARSING) ?? "";
				IDC_EXPLORE.Enabled = !(psi is null);
			}
		}
		...
		private class CFillResultsOnBackgroundThread
		{
			...
			public void StartThread(IResultsFolder prf)
			{
				...
				ThreadPool.QueueUserWorkItem(delegate
				{
					...
					if (hr.Succeeded && ppv is IResultsFolder lprf)
					{
