	...
	unsafe static bool AddAccessRights(string szPrinterName, uint dwAccessMask, AceFlags bAceFlags, string szUserName, byte bType)
	{
		...
		try
		{
			...
			if (bType == ACCESS_DENY)
			{
				...
				{
					Console.Write("Error {0}: AddAccessDeniedAce\n", Win32Error.GetLastError());
					throw new Exception();
				}
			...
			if (bType == ACCESS_ALLOW)
			{
				...
				{
					Console.Write("Error {0}: AddAccessAllowedAce\n", Win32Error.GetLastError());
					throw new Exception();
				}
			...
			{
				Console.Write("Error {0} setting printer DACL\n", Win32Error.GetLastError());
				throw new Exception();
			}
		}
		finally
