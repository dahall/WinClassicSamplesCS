...
static class CertSign
{
	...
	*****************************************************************************/
	...
	static HRESULT HrCreateCNGHash(string wszHashAlgName, IntPtr pbData, uint cbData, out SafeLocalHandle ppbHash)
	{
		...
		using (hAlgorithm)
		{
			...
			//create hash
			hr = BCryptCreateHash(hAlgorithm, out var hHash, pbHashObject, cbHashObject);
			if (hr.Failed)
			{
				goto CleanUp;
			}
	...
	static HRESULT HrSignCNGHash(NCRYPT_KEY_HANDLE hKey, IntPtr pPaddingInfo, EncryptFlags dwFlag, IntPtr pbHash, uint cbHash, out SafeLocalHandle ppbSignature)
	{
		...
		HRESULT hr = NCryptSignHash(hKey, pPaddingInfo, pbHash, cbHash, default, 0, out var pcbSignature, dwFlag);
		...
		hr = NCryptSignHash(hKey, pPaddingInfo, pbHash, cbHash, ppbSignature, pcbSignature, out _, dwFlag);
	...
	{
		HRESULT hr = HRESULT.S_OK;
		SafeHCRYPTHASH hHash = SafeHCRYPTHASH.Null;
	...
	*****************************************************************************/
	static unsafe int Main(string[] args)
	{
		...
		SafeLocalHandle pbPlainText = SafeLocalHandle.Null;

		//certificate to be used to sign data
		SafePCCERT_CONTEXT pCertContext = SafePCCERT_CONTEXT.Null;
		...
		SafeLocalHandle pbHash = SafeLocalHandle.Null;
		SafeLocalHandle pbSignature = SafeLocalHandle.Null;
		...
		if (fSign)
		{
			...
			Console.Write("Created signature file: {0}\n", pwszSignatureFile);
		}
		else
		{
