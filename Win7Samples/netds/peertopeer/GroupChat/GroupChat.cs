...
public partial class GroupChat : Form
{
	...
	internal static void BrowseHelper(IWin32Window hDlg, TextBox idEditbox, string pwzFileType, string pwzFileExtension, bool fOpen)
	{
		...
		FileDialog fd = fOpen ? new OpenFileDialog() : new SaveFileDialog();
		fd.Filter = filter;
	...
	internal HRESULT OpenGroup(string? pwzIdentity, string? pwzName)
	{
		...
		{
			DisplayHrError("Please provide an identity.", hr = HRESULT.E_INVALIDARG);
		}
		...
		if (hr.Succeeded)
		{
			hr = Main?.PrepareToChat() ?? HRESULT.E_OUTOFMEMORY;
		}
	...
	{
		var pwzIdentity = GetSelectedChatMember() ?? throw new InvalidOperationException("No selected member");
