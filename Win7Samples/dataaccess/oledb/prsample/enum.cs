...
public static partial class Program
{
	...
	private static void myCreateEnumerator(in Guid clsidEnumerator, out Guid pCLSID)
	{
		...
		//ushort[] wszProgID = new[MAX_NAME_LEN + 1];
		...
		myDisplayRowset(pIRowset!, "SOURCES_NAME", out var wszProgID).ThrowIfFailed();
		...
		if (!myGetInputFromUser(out var wszProgID2, "\nType the ProgID of a provider to use [Enter = `{0}`]: ", wszProgID))
			wszProgID2 = wszProgID;
		CLSIDFromProgID(wszProgID2!, out pCLSID);
	}
}