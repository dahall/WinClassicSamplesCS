...
internal partial class CTSFEditWnd : TextBox, ITextStoreACP, ITfContextOwnerCompositionSink, ITfFunctionProvider
{
	...
	public IEnumerable<(string, string?)> ReadingText
	{
		get
		{
			...
			*/
			foreach (ITfRange pPropRange in new Vanara.Collections.IEnumFromCom<ITfRange>(pEnumRanges.Next, pEnumRanges.Reset))
//				foreach (ITfRange pPropRange in new Vanara.Collections.IEnumFromCom<ITfRange>(pEnumRanges))
			{
				...
				*/
				...
				//while (celtFetched > 0)
				foreach (TF_PROPERTYVAL rgelt in new Vanara.Collections.IEnumFromCom<TF_PROPERTYVAL>(pEnumPropertyVal.Next, pEnumPropertyVal.Reset))
				{
	...
	public IEnumerable<Guid> TextOwner => GetPropVals<uint>(GUID_PROP_TEXTOWNER).Select(m_pCategoryMgr!.GetGUID);
