...
internal class CLangBarItemButton : ITfLangBarItemButton, ITfSource
{
	...
	public static readonly int CONNECT_E_ADVISELIMIT = CONNECT_E_FIRST + 1;

	// connection attempt failed
	public static readonly int CONNECT_E_CANNOTCONNECT = CONNECT_E_FIRST + 2;

	// there is no connection for this connection id
	public static readonly int CONNECT_E_NOCONNECTION = CONNECT_E_FIRST;

	// must use a derived interface to connect
	public static readonly int CONNECT_E_OVERRIDDEN = CONNECT_E_FIRST + 3;
	...
	{
		if (!Equals(typeof(ITfLangBarItemSink).GUID, riid))
			throw new COMException(null, CONNECT_E_CANNOTCONNECT);

		if (pLangBarItemSink is not null)
