...
public abstract class CAppMessageLoop
{
	...
	private nuint _uPostTimerId;
	private nuint _uTimeoutTimerId;      // timer id used to exit the app if the app is not called back within a certain time
	...
	protected void MessageLoop()
	{
		...
		while (GetMessage(out var msg, default, 0, 0) != 0)
		{
			...
			{
				KillTimer(default, msg.wParam.ToUIntPtr());    // all are one shot timers

				if (msg.wParam.ToUIntPtr() == _uTimeoutTimerId)
				{
					_uTimeoutTimerId = default;
				}
				else if (msg.wParam.ToUIntPtr() == _uPostTimerId)
				{
