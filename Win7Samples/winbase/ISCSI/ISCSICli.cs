...
internal static class ISCSICli
{
	...
	*/
	private static bool DiscpIsDeviceNumberInVolume(uint DeviceNumber, in VOLUME_DISK_EXTENTS Volume)
	{
	...
	*VolumePathLen returns with the number of characters in the
	...
	public static readonly Guid WDI_STORAGE_PREDICT_FAILURE_DPS_GUID = new(0xe9f2d03a, 0x747c, 0x41c2, 0xbb, 0x9a, 0x02, 0xc6, 0x2b, 0x6d, 0x5f, 0xcb);
	...
	*/
	...
	private static Win32Error PSKey(string[] ArgV)
	{
		...
		uint KeyLength = 0;
		bool Persist;
		string? Initiator = ArgV[1].StartsWith('*') ? null : ArgV[1];
		uint InitiatorPort = ArgV[2].StartsWith('*') ? ISCSI_ALL_INITIATOR_PORTS : uint.Parse(ArgV[2]);
		var SecurityFlags = (ISCSI_SECURITY_FLAGS)int.Parse(ArgV[3]);
		var IdType = (IKE_IDENTIFICATION_PAYLOAD_TYPE)int.Parse(ArgV[4]);
		var IdText = ArgV[5];
		Win32Error Status = Win32Error.ERROR_SUCCESS;
	...
	private static Win32Error DoLoginToTarget(string[] ArgV, bool IsPersistent)
	{
		...
		else
		{
			...
			using SafeCoTaskMemStruct<ISCSI_LOGIN_OPTIONS> pLoginOptions = LoginOptions;
			Status = LoginIScsiTarget(TargetName, !ReportToPNP, InitiatorInstance, PortNumber,
				pTargetPortal, SecurityFlags, (IntPtr)Mapping, pLoginOptions, KeyLength, Key, IsPersistent, out ISCSI_UNIQUE_SESSION_ID SessionId, out ISCSI_UNIQUE_SESSION_ID ConnectionId);
	...
	{
		Win32Error Status;
		string TargetPortalAddress, TargetPortalSocket;
		string? HBAName;
		uint PortNumber;
