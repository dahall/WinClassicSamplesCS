...
public class OpticalStorageEraseMediaOperation : OpticalStorageMediaOperation<IDiscFormat2Erase>
{
	...
	/// cref="SupportsDevice"/> method.
	/// </remarks>
	protected override IDiscRecorder2? Recorder { get => op.Recorder; set => op.Recorder = value ?? throw new InvalidOperationException("A recorder is required for this operation."); }
	...
	public override void Execute()
	{
		...
		if (string.IsNullOrEmpty(op.ClientName))
			op.ClientName = AppDomain.CurrentDomain.FriendlyName;
...
public abstract class OpticalStorageMediaOperation<T> : IOpticalStorageMediaOperation where T : IDiscFormat2
{
	...
	{
		op = opInst;
		ClientName = string.IsNullOrEmpty(clientName) ? AppDomain.CurrentDomain.FriendlyName : clientName;
	}
