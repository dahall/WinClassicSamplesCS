...
{
	/// <summary>
	/// Takes a WMI object path and escapes it so that it can be used inside a WQL query WHERE clause. This effectively means replacing
	/// '\' and '"' characters so they are treated like any other characters.
	/// </summary>
	/// <param name="objectPath">The object management path.</param>
	/// <returns>The escaped object management path.</returns>
	public static string EscapeObjectPath(string objectPath) => objectPath.Replace("\\", "\\\\").Replace("\"", "\\\"");

	/// <summary>Gets the first object in a collection of ManagementObject instances.</summary>
	...
	public static ManagementObject? GetFirstObjectFromCollection(ManagementObjectCollection collection) =>
		collection.Cast<ManagementObject>().FirstOrDefault();

	/// <summary>Gets the Msvm_ComputerSystem instance that matches the host computer system.</summary>
	...
	public static ManagementObject? GetHostComputerSystem(ManagementScope scope) =>
		// The host computer system uses the same WMI class (Msvm_ComputerSystem) as the virtual machines, so we can simply reuse the
		// GetVirtualMachine with the name of the host computer system.
		GetVirtualMachine(Environment.MachineName, scope);

	/// <summary>Gets the Msvm_ComputerSystem instance for the host computer system with name hostName.</summary>
	...
	public static ManagementObject? GetHostComputerSystem(string hostName, ManagementScope scope) =>
		// The host computer system uses the same WMI class (Msvm_ComputerSystem) as the virtual machines, so we can simply reuse the
		// GetVirtualMachine with the name of the host computer system.
		GetVirtualMachine(hostName, scope);

	/// <summary>Gets the Msvm_PlannedComputerSystem instance matching the requested virtual machine name.</summary>
	...
	public static ManagementObject? GetPlannedVirtualMachine(string name, ManagementScope scope) => GetVmObject(name, "Msvm_PlannedComputerSystem", scope);

	/// <summary>Gets the CIM_ResourcePool derived instance matching the specified type, subtype and pool id.</summary>
	...
	public static ManagementObject? GetResourcePool(string resourceType, string resourceSubtype, string poolId, ManagementScope scope)
	{
		...
		SelectQuery poolQuery = new(poolQueryWql);

		using ManagementObjectSearcher poolSearcher = new(scope, poolQuery);
	...
	/// <summary>Gets the CIM_ResourcePool derived instances matching the specified type, and subtype.</summary>
	/// <param name="resourceType">The resource type of the resource pool.</param>
	/// <param name="resourceSubtype">The resource subtype of the resource pool.</param>
	/// <param name="scope">The ManagementScope to use to connect to WMI.</param>
	/// <returns>The CIM_ResourcePool derived instance.</returns>
	...
	/// <summary>Gets the array of Msvm_StorageAllocationSettingData of VHDs associated with the virtual machine.</summary>
	...
	/// <summary>Gets the array of Msvm_StorageAllocationSettingData of VHDs associated with the given virtual machine settings.</summary>
	/// <param name="virtualMachineSettings">A ManagementObject representing the settings of a virtual machine or snapshot.</param>
...
#pragma warning disable CS8625 // Cannot convert null literal to non-nullable reference type.
		if (virtualMachineSettings is not null)
				...
				null, null, null, null, false, null))
		{
			...
			{
				if ((ushort)sasd["ResourceType"] == SASDResourceTypeLogicalDisk)
				{
					sasdList.Add(sasd);
				}
				else
...
#pragma warning restore CS8625 // Cannot convert null literal to non-nullable reference type.
	...
	/// <summary>Gets the Msvm_ComputerSystem instance that matches the requested virtual machine name.</summary>
	/// <param name="name">The name of the virtual machine to retrieve the path for.</param>
	/// <param name="scope">The ManagementScope to use to connect to WMI.</param>
	/// <returns>The Msvm_ComputerSystem instance.</returns>
	public static ManagementObject? GetVirtualMachine(string name, ManagementScope scope) => GetVmObject(name, "Msvm_ComputerSystem", scope);
	...
	{
		using ManagementClass managementServiceClass = new("Msvm_VirtualSystemManagementService") { Scope = scope };
		return GetFirstObjectFromCollection(managementServiceClass.GetInstances());
	}

	/// <summary>Gets the virtual system management service setting data.</summary>
	/// <param name="scope">The scope to use when connecting to WMI.</param>
	...
	{
		using ManagementClass serviceSettingsClass = new("Msvm_VirtualSystemManagementServiceSettingData") { Scope = scope };
		return GetFirstObjectFromCollection(serviceSettingsClass.GetInstances());
	}

	/// <summary>Gets the virtual machine's configuration settings object.</summary>
	...
	{
#pragma warning disable CS8625 // Cannot convert null literal to non-nullable reference type.
		using ManagementObjectCollection settingsCollection = virtualMachine.GetRelated("Msvm_VirtualSystemSettingData",
			"Msvm_SettingsDefineState", null, null, null, null, false, null);
#pragma warning restore CS8625 // Cannot convert null literal to non-nullable reference type.
		return GetFirstObjectFromCollection(settingsCollection);
	}

	/// <summary>Gets the virtual system snapshot service.</summary>
	/// <param name="scope">The scope to use when connecting to WMI.</param>
	...
	{
		using ManagementClass snapshotServiceClass = new("Msvm_VirtualSystemSnapshotService") { Scope = scope };
		return GetFirstObjectFromCollection(snapshotServiceClass.GetInstances());
	}

	/// <summary>Prints the relevant message from embedded instances of Msvm_Error.</summary>
	...
	public static void PrintMsvmErrors(ManagementObject job)
	{
		...
		using (ManagementBaseObject outParams = job.InvokeMethod("GetErrorEx", inParams, new()))
		{
	...
	/// <summary>Validates the output parameters of a method call and prints errors, if any.</summary>
	...
	public static bool ValidateOutput(ManagementBaseObject outputParameters, ManagementScope scope, bool throwIfFailed = true, bool printErrors = false)
	{
		...
		if ((uint)outputParameters["ReturnValue"] == 4096)
		{
			...
			using ManagementObject job = new((string)outputParameters["Job"]);
			job.Scope = scope;
			...
			if (!IsJobSuccessful(job["JobState"]))
			{
				...
				// In some cases the Job object can contain helpful information about why the method call failed. If it did contain
				// such information, use it instead of a generic message.
	...
	/// <summary>Gets the first virtual machine object of the given class with the given name.</summary>
	/// <param name="name">The name of the virtual machine to retrieve the path for.</param>
	...
	private static ManagementObject? GetVmObject(string name, string className, ManagementScope scope)
	{
		...
		SelectQuery vmQuery = new(vmQueryWql);

		using ManagementObjectSearcher vmSearcher = new(scope, vmQuery);
		...
		// If multiple virtual machines exist with the requested name, return the first one.
		return GetFirstObjectFromCollection(vmCollection);
	}
	...
	private static bool IsJobComplete(object jobStateObj) =>
		(JobState)(ushort)jobStateObj is JobState.Completed or JobState.CompletedWithWarnings or JobState.Terminated or JobState.Exception or JobState.Killed;
	...
	private static bool IsJobSuccessful(object jobStateObj) => (JobState)(ushort)jobStateObj is JobState.Completed or JobState.CompletedWithWarnings;
}